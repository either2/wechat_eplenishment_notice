...
class add_stockurl:
    ...
    def data_unite(self, openid, stock_url, api_url, stock_message,database):
        stcok_urk_list = self.read_data("SELECT stock_url FROM %s "%database)
        if stock_url in stcok_urk_list:
            openid_list = self.read_data("SELECT openid FROM %s WHERE stock_url='%s'" % (database,stock_url))
            if openid in openid_list[0]:
                return "您已关注这个商品"
            else:
                self.up_data(
                    "UPDATE %s SET openid=CONCAT(openid,';%s')WHERE stock_url='%s'" % (database,openid, stock_url))
                return "关注成功，商品补货后会第一时间通知您的哦"
        else:
            stock_message = stock_message.replace("'", "")
            website_name=re.findall("[a-z]+\.com",stock_url)[0].split(".com")[0]
            self.up_data(
                "INSERT INTO  %s (stock_url,openid,api_url,stock_message,website) VALUES('%s','%s','%s','%s','%s')" % (database,
    ...
    def add_stockurl(self, openid, stock_url, api_url, stock_message,database):
        ...
        if openid not in openid_list:
            ...
            openid, stock_url))
            return self.data_unite(openid, stock_url, api_url, stock_message,database)
        ...
        def text_message(message):
            ...
            if re.findall("丝芙兰" + '.*?' + 'sephora.com' + '.*?' + 'P' + "[0-9]+", message.content):
                ...
                if sql_stock_message == None:
                    data = {
                        "url": "%s" % stock_url}
                    url = "http://45.32.110.62:2333/stockmessage"
